...
public class EventConfigSOEditor : Editor
{
    ...
    private void DrawSubscribeToolsCorrect(Type selfType)
    {
        ...
        for (int i = 0; i < _config.subscribeBinds.Count; i++)
        {
            ...
            // 2. 选择目标事件里的方法
            using (new EditorGUI.DisabledScope(string.IsNullOrEmpty(sub.targetEventClassName)))
            {
                if (GUILayout.Button("② 选择目标事件方法", GUILayout.Height(26)))
                {
                    Type targetType = FindEventType(sub.targetEventClassName);
    ...
    // 选择监听的事件类
    private void ShowEventClassMenu(EventSubscribeBind sub)
    {
        ...
        foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
        {
            ...
            {
                if (t.IsClass && !t.IsAbstract && typeof(IBaseEvent).IsAssignableFrom(t))
                {
                    m.AddItem(new GUIContent(t.Name), false, () =>
                    {
                        sub.targetEventClassName = t.Name;
                        SaveRefresh();
                    });
                }
            }
    ...
    private void ShowEventMethodMenu(Type targetType, EventSubscribeBind sub)
    {
        GenericMenu m = new GenericMenu();
        foreach (var mi in targetType.GetMethods(BindingFlags.Instance | BindingFlags.Public ))
        {
            if (mi.ReturnType == typeof(void))
            {
                ...
                {
                    sub.methodNameInTargetEvent = mi.Name;
                    SaveRefresh();
                });
            }
        }
    ...
    private void ShowLocalExecuteMethodMenu(Type selfType, EventSubscribeBind sub)
    {
        GenericMenu m = new GenericMenu();
    ...
    private void ShowValidMethodMenu(Type eventType, EventMethodBind bind, bool isCallback = false)
    {
        GenericMenu m = new GenericMenu();
