namespace UniVCon
{
    ...
    public class YourAction1 : BaseAction {
        ...
        [Preserve] public async UniTask SpawnLabelList(object data) {
            if (!TryConvertData(data, out List<LabelData> dataList)) return;
        ...
        [Preserve] public UniTask SendCreateLabel(object data) {
            var labelList = new List<LabelData> {
                new LabelData {
                    identifyID = "UnityLabel1", prefabKey = "", title = "Unity label title 1", desc = "Unity label desc 1", deviceName = "Unity device A"
                }
            };
            MessageSender.SendActionMessage( type: WebMsgHandlerManager.MessageType, actionName: "YourAction1", funcName: "SpawnLabelList", data: labelList);
            return UniTask.CompletedTask;
        }
        public void SendUpdateLabel(List<LabelData> labelList) {
            MessageSender.SendCurrentMessage( type: WebMsgHandlerManager.MessageType, funcName: "UpdateLabel", data: labelList);
        }
