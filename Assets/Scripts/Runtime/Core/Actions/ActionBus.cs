namespace UniVCon
{
    ...
    public class ActionBus : IActionBus {
        ...
        public void Publish<T>(T message) {
            ...
            var type = typeof(T);
            lock (_sync) {
                if (!_subscribers.TryGetValue(type, out var callbacks) || callbacks.Count == 0) return;
                snapshot = new List<Delegate>(callbacks);
            }
            foreach (var del in snapshot) {
                try {
                    if (del is Action<T> callback) callback(message);
                }
                catch (Exception e) {
        ...
        public void UnSubscribe<T>(Action<T> callback) {
            ...
            lock (_sync) {
                ...
                if (callbacks.Count == 0) _subscribers.Remove(type);
            }
        }
        private sealed class Subscription<T> : IDisposable {
            ...
            public void Dispose() {
                ...
                if (bus == null || callback == null) return;
                _bus = null;
                _callback = null;
