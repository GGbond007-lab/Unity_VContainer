...
{
    [Test]
    public async System.Threading.Tasks.Task ReceiveMessageFromWebReturnsInvalidJson()
    {
    ...
    [Test]
    public async System.Threading.Tasks.Task ReceiveMessageFromWebReturnsMissingAction()
    {
        var sender = new CapturingMessageSender();
        var manager = new WebMsgHandlerManager(new ActionDispatcher((_, _) => new TestAction(), new ActionStack(), new TestActionConfigProvider()), sender);
    ...
    [Test]
    public async System.Threading.Tasks.Task ReceiveMessageFromWebRejectsUnsupportedType()
    {
        var sender = new CapturingMessageSender();
        var manager = new WebMsgHandlerManager(new ActionDispatcher((_, _) => new TestAction(), new ActionStack(), new TestActionConfigProvider()), sender);
    ...
    {
        public ActionExecutionResult LastError { get; private set; }
