...
{
    [Test]
    public async System.Threading.Tasks.Task DispatchReturnsStructuredErrorWhenActionFactoryThrows()
    {
        ...
        var configProvider = new TestActionConfigProvider(new[] { config });

        var dispatcher = new ActionDispatcher(
            (_, _) => throw new InvalidOperationException("resolve failed"),
            new ActionStack(),
            configProvider);
    ...
    [Test]
    public async System.Threading.Tasks.Task DispatchToCurrentUsesStackTopWhenActionNameIsEmpty()
    {
        ...
        var dispatcher = new ActionDispatcher(
            (_, _) => throw new AssertionException("Factory should not be called."),
            stack,
            new TestActionConfigProvider());
