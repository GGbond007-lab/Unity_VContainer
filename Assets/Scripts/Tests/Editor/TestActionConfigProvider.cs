...
public sealed class TestActionConfigProvider : IActionConfigProvider
{
    ...
    public ActionConfigSO GetConfigByTargetAction(Type targetType)
    {
        ...
        if (_configByTargetType.TryGetValue(targetType.FullName, out var config))
            return config;
    ...
    public ScriptableObject GetScriptableObjectByType(Type soType)
    {
        ...
        {
            if (so != null && (so.GetType() == soType || soType.IsAssignableFrom(so.GetType())))
                return so;
        }
    ...
    private void RegisterConfig(ActionConfigSO config)
    {
        ...
        if (!string.IsNullOrEmpty(config.actionName))
            _configByActionName[config.actionName] = config;

        if (!string.IsNullOrEmpty(config.targetActionClassName))
            _configByTargetType[config.targetActionClassName] = config;

        if (config.requiredSOs == null)
            return;
