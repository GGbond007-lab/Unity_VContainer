namespace UniVCon.Tests
{
    ...
    public sealed class ActionConfigProviderTests {
        [Test] public void TestProvidersKeepIndependentConfigState() {
            ...
            var first = new TestActionConfigProvider(new[] {
                config
            }
            );
        ...
        [Test] public void GetConfigByTargetActionAcceptsFullNameAndTypeNameKeys() {
            var config = ScriptableObject.CreateInstance<ActionConfigSO>();
            config.actionName = "Test";
            config.targetActionClassName = nameof(TestAction);
            var provider = new TestActionConfigProvider(new[] {
                config
            }
            );
