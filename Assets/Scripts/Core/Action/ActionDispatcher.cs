...
public sealed class ActionDispatcher
{
    ...
    public async UniTask<ActionExecutionResult> Dispatch(string actionName, string funcName, object data)
    {
        ...
        if (config == null)
        {
            return ActionExecutionResult.Fail(
                ...
                $"Action '{actionName}' is not configured.",
                actionName,
                funcName);
        }
        ...
        if (actionType == null)
        {
            return ActionExecutionResult.Fail(
                ActionErrorCode.ActionNotFound,
                $"Action type '{config.targetActionClassName}' is not registered.",
                actionName,
                funcName);
        }
        ...
        else
        {
            ...
            if (!pushResult.Success && pushResult.ErrorCode != ActionErrorCode.DuplicateAction)
                return pushResult;
        ...
        if (string.IsNullOrEmpty(funcName))
            return ActionExecutionResult.Ok(actionName, funcName);
    ...
    private async UniTask<ActionExecutionResult> DispatchToCurrent(string funcName, object data)
    {
        ...
        if (currentAction == null)
        {
            return ActionExecutionResult.Fail(
                ActionErrorCode.ActionNotFound,
                "There is no current action on the stack.",
                null,
                funcName);
        }
